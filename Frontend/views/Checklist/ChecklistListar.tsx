  ...
  useEffect(() => {
    ...
    fetch(
      ...
      .then((apiResponse) => {
        ...
        const chklstParser = z.array(ListadoChecklistDiarioApiParser);
        const chcklst: ListadoChecklistDiarioType[] =
          chklstParser.parse(apiResponse);
      ...
      <GeneralContainer title="Listado de Checklists por Vehículo">
        ...
        <TableContainer title="Listado de Checklists">
          {metadataPage.data.length > 0 ? (
            <TableResponsive
              tableData={metadataPage.data.map((data) => (
                <tr key={data.IdChecklistDiario} className="text-center">
                  <td>{ParserDatesToStringDateOnly(new Date(data.Fecha))}</td>
                  <td>
                    {data.FaroDelanteroIzquierdo ? (
                      <svg
